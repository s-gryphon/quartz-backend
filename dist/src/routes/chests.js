...
async function getChestInProject(projectId, chestId) {
 ...
 */
chestRoutes.post('/:projectId/chests', requireProjectMember('edit'), zValidator('json', createChestSchema), async (c) => {
    const projectId = c.req.param('projectId');
    ...
    try {
        ...
        if (body.contents?.length) {
            await db.insert(chestContents).values(body.contents.map((item) => ({
                chestId: chest.id,
                slot: item.slot,
                itemId: item.item_id,
                count: item.count,
 ...
 */
chestRoutes.get('/:projectId/chests/:chestId', requireProjectMember('view'), async (c) => {
    const projectId = c.req.param('projectId');
    const chestId = c.req.param('chestId');
    const chest = await getChestInProject(projectId, chestId);
    if (!chest)
        throw notFound('Chest');
 ...
 */
chestRoutes.patch('/:projectId/chests/:chestId', requireProjectMember('edit'), zValidator('json', updateChestSchema), async (c) => {
    const projectId = c.req.param('projectId');
    const chestId = c.req.param('chestId');
    const body = c.req.valid('json');
 ...
 */
chestRoutes.delete('/:projectId/chests/:chestId', requireProjectMember('edit'), async (c) => {
    const projectId = c.req.param('projectId');
    const chestId = c.req.param('chestId');
    const existing = await getChestInProject(projectId, chestId);
    if (!existing)
 ...
 */
chestRoutes.put('/:projectId/chests/:chestId/contents', requireProjectMember('edit'), zValidator('json', replaceContentsSchema), async (c) => {
    const projectId = c.req.param('projectId');
    const chestId = c.req.param('chestId');
    ...
    if (contents.length > 0) {
        await db.insert(chestContents).values(contents.map((item) => ({
            chestId,
            slot: item.slot,
            itemId: item.item_id,
            count: item.count,
