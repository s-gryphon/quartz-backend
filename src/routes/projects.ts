...
function serializeProject(project: typeof projects.$inferSelect, role?: string) {
  return {
    id: project.id,
    name: project.name,
...
projectRoutes.patch(
  ...
  async (c) => {
    ...
    if (userId === currentUser.id && body.role !== 'admin') {
      return c.json(
        { error: { code: 'BAD_REQUEST', message: 'Cannot demote yourself' } },
        400,
      )
    }
...
projectRoutes.delete('/:projectId/members/:userId', requireProjectMember('admin'), async (c) => {
  ...
  if (userId === currentUser.id) {
    return c.json(
      { error: { code: 'BAD_REQUEST', message: 'Cannot remove yourself from the project' } },
      400,
    )
  }
